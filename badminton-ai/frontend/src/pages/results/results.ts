...
const jobTitle = document.getElementById("jobTitle") as HTMLHeadingElement;
const jobMeta = document.getElementById("jobMeta") as HTMLParagraphElement;
const confidenceBadge = document.getElementById("confidenceBadge") as HTMLSpanElement;

const kpiAvgSpeed = document.getElementById("kpiAvgSpeed") as HTMLSpanElement;
const kpiMaxSpeed = document.getElementById("kpiMaxSpeed") as HTMLSpanElement;
const kpiShots = document.getElementById("kpiShots") as HTMLSpanElement;
const kpiRallies = document.getElementById("kpiRallies") as HTMLSpanElement;
...
const coachingLoading = document.getElementById("coachingLoading") as HTMLDivElement;
const coachingBody = document.getElementById("coachingBody") as HTMLDivElement;
const coachingSummary = document.getElementById("coachingSummary") as HTMLParagraphElement;
const coachingStrengths = document.getElementById("coachingStrengths") as HTMLUListElement;
const coachingRecommendations = document.getElementById("coachingRecommendations") as HTMLUListElement;
const coachingProviderBadge = document.getElementById("coachingProviderBadge") as HTMLSpanElement;
const regenerateCoachingBtn = document.getElementById("regenerateCoachingBtn") as HTMLButtonElement;

const shotsTableBody = document.getElementById("shotsTableBody") as HTMLTableSectionElement;
const replayVideo = document.getElementById("replayVideo") as HTMLVideoElement;
const downloadVideoBtn = document.getElementById("downloadVideoBtn") as HTMLButtonElement;
const downloadReplayBtn = document.getElementById("downloadReplayBtn") as HTMLButtonElement;

function bulletList(list: HTMLUListElement, items: string[], emptyText: string): void {
...
function renderBarChart(strokeCounts: Record<string, number>): void {
  const canvas = document.getElementById("strokeChart") as HTMLCanvasElement;
...
function renderPieChart(strokeCounts: Record<string, number>): void {
  const canvas = document.getElementById("strokePieChart") as HTMLCanvasElement;
...
async function renderCoaching(token: string, force: boolean): Promise<void> {
...
function renderShots(shots: Awaited<ReturnType<typeof jobsApi.shots>>): void {
  if (shots.length === 0) {
...
async function renderResults(job: AnalysisJobRead, token: string): Promise<void> {
  ...
  if (player0Key) (document.getElementById("heatmap0") as HTMLImageElement).src = await loadAuthedObjectUrl(`/jobs/${job.id}/heatmaps/${player0Key}`, token);
  if (player1Key) (document.getElementById("heatmap1") as HTMLImageElement).src = await loadAuthedObjectUrl(`/jobs/${job.id}/heatmaps/${player1Key}`, token);
  ...
  (document.getElementById("openReplayLink") as HTMLAnchorElement).href = `../replay/index.html?jobId=${job.id}`;

  regenerateCoachingBtn.addEventListener("click", () => void renderCoaching(token, true));
  void renderCoaching(token, false);
